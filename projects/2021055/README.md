...
### SemA.html - SemC.html
Η λογική για την δημιουργία των πινάκων είναι ακριβώς η ιδία και στα δύο αρχεία. Αρχικά, δημιούργησα έναν πίνακα σε γλώσσα liquid, ο οποίος περιέχει με την σειρά, τις ημέρες που πραγματοποιούνται οι εξετάσεις των μαθημάτων. Για να χωριστούν με την σωστή σειρά, όταν καλείται ο πίνακας, χωρίζει τα στοιχεία του με βάση το κόμμα(,).Στην συνέχεια, χρησιμοποιώ την εντολή `table` της γλώσσας html γι να δημιουργήσω τον πίνακα. Με την εντολή `thead` εισάγω όλες τις απαιρέτητες επικεφαλίδες(ΩΡΑ,ΗΜΕΡΑ κ.α). Για την προσθήκη περιεχομένου στον πίνακα, χρησιμοποιείται η εντολή `tbody`. Μέσα σε αυτή, για την εισαγωγή των στοιχείων με την σωστή σειρά χρησιμοποιείται μια διπλή for της γλώσσας liquid. Πιο συγκεκριμένα, στο repository all collections, στον φάκελο courses, ο οποίος περιέχει τα μαθήματα του πανεπηστημίου, έχουν προσθετεί στα αρχεία του νεές μεταβλητές. Η μεταβλητή `IN` δείχνει την σειρά με την οποία εξετάστηκαν τα μαθήματα, η μεταβλητή `DATE` δήλωνει την ημερμομηνία που εξετάστηκαν και η μεταβλητή `HOUR` δηλώνει την ώρα εξέτασης τους. Η πρώτη for σκανάρει τον φάκελο `courses`, εάν υπάρχει η μεταβλητή `IN` σε κάποιο αρχείο( {% if i.IN %} ), τότε ο μετρητής αυξάνεται κατα ένα({% assign count = count | plus: 1 %}), τότε η επόμενη for ξανα σκανάρει τον φάκελο `courses` και άν ο αριθμός τηε μεταβλητής count είναι ίσος με τον αριθμό που έχει σε εκείνο το πέρασα η μεταβλητή `IN`({% if count == i.IN %}), προστήθεται στον πίνακα με σειρά η ημερομηνία εξέτασης, η ημέρα εξέτασηςν η ώρα εξέτασης, το εξεταζόμενο μάθημα και ο διδάσκων του μαθήματος.
